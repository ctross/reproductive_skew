...
###################################################################### Load Data
 ...
 dnh <- read.csv("ReproSkewNHMPublishedData-Paired-Male.csv")

 dnh$N[13] = 43 # Missing sample, so assume that its the same as the horse sample in the same study.
...
dnh$M_Posterior_Estimate[i] <- M_index_from_B_index(dnh$B[i],dnh$R[i],dnh$N[i])
 ...
 dnh$N0 <- round(dnh$N,0)
########################################
 setwd(paste0(basewd,'/Results'))
 write.csv(dnh,"SkewResults-Published-NHM-Paired-Male.csv")
 setwd(basewd)
